...
<!-- The binding document for the overnight loop harness (planned): a fresh agent is
     launched against this brief every time the previous one stops, so it must be
     self-contained and STABLE — the live "what next" belongs in run_status.md (see §2),
     not here. Copy into docs/<game>/ and fill in every <...>. -->
...
- [ ] The game is **fully runnable** in the VM: bring-up complete, no-op
      frame verify passes, the input-wait registry is stable (demos replay
      identically under every driver — no freezes).
...
- [ ] The demo suite runs **headless and fast enough** to gate every slice
      (PyPy is the speed path if replays crawl).
- [ ] A **lifter census** exists (`python c64_re/tools/liftgen.py IMAGE
      [--frames N] [--scan-jsr LO:HI] [--manifest PATH]`), so the work queue
...
building any tooling (it probably exists; check `docs/roadmap.md` and `c64_re/` too).

## 3. Unattended safety (non-negotiable — the loop protocol)
...
<!-- The commands a slice must pass before its commit, e.g.:
     python -m pytest tests -q            (port suite, incl. determinism digests)
     python -m pytest ../c64_re/tests -q  (framework suite, when core was touched)
     python c64_re/tools/lint.py; python c64_re/tools/audit_layers.py <pure layers>
...
<!-- Ordered buckets of demand-driven targets, coarse enough to stay valid for weeks:
     ...
     2. <e.g. SID register-stream layer N>
