...
block before finishing. `README.md`/`docs/roadmap.md` route here; the loop
protocol (smallest slice, never commit red, revert + log blockers) applies to
every prompt.
...
VERIFIED. The ladder is inherited from the DOS lineage
(`dos_recosystem/dos_re/islands.py`); the C64 encoding is
`c64_re.islands` (`@oracle_link`, its `STATUSES` ladder) — this block and the
code must agree.

## The one warning that matters

**AI can be confidently wrong about semantics.** In the DOS lineage, on
Prehistorik 2, an agent once interpreted bridge tiles bending under the player
as the player *eating something* — a fluent, plausible, completely false
story. That is why the oracle stays central: without it you guess; with it you
are forced to converge. Name things by evidence, verify before you trust, and
treat your own narrative sense as a hypothesis generator, never a source.
...
| [`create_state_mirror.md`](create_state_mirror.md) | Name an island's addresses behind typed views without weakening verification. |
...
| [`prove_native_equivalence.md`](prove_native_equivalence.md) | Endgame: build the `c64_re.tick_demo` adapter and prove the VM-less core replays the whole corpus tick-exact. |
